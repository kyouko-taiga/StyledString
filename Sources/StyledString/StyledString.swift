...
public struct StyledString: CustomStringConvertible, ExpressibleByStringLiteral {
  ...
  public init(_ value: String) throws {
    ...
    while cursor != value.endIndex {
      ...
      switch token {
      case "{" where !isParsingExpression:
        // Escape pairs of `{`.
      ...
      case "}" where isParsingExpression:
        ...
        try styled.append(StyledString.format(expression))
  ...
  private static func format(_ expression: String) throws -> String {
    let data = expression.split(separator: ":", maxSplits: 2, omittingEmptySubsequences: true)
    assert(data.count <= 2)
    var subject = String(data.first ?? "")
    let styles = (data.last ?? "")
      .split(separator: ",")
    ...
    for components in styles {
      ...
      subject = try style.apply(on: subject, parameters: Array(components.dropFirst()))
    }
